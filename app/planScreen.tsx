...
const stripDashes = (text: string) => text.replace(/^---+$/gm, "").trim();
...
const SECTION_KEYS = Object.keys(SESSION_ICONS);
...
const parseDay = (dayText: string): ParsedDay => {
  ...
  const dayNumber = dayNumMatch ? parseInt(dayNumMatch[1], 10) : 1;
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (sessionType) {
      ...
      // Non-bullet content lines treated as drill items too
      currentSession.items.push(parseDrillLine(line));
    }
  }
  ...
  // If the AI gives a rest day without section headers, still show it cleanly.
  if (sessions.length === 0 && /rest|recovery/i.test(dayText)) {
    sessions.push({
      type: dayText.toLowerCase().includes("recovery") ? "active recovery" : "rest",
      icon: dayText.toLowerCase().includes("recovery") ? SESSION_ICONS["active recovery"] : SESSION_ICONS.rest,
      duration: "",
      items: [{ name: theme || "Rest Day", detail: "Recover and prepare for the next session." }],
    });
  }
...
const findClosingIndex = (planText: string): number => {
  const lower = planText.toLowerCase();
  const indexes = CLOSING_MARKERS
...
const getPlanWithoutClosingText = (planText: string): string => {
  const idx = findClosingIndex(planText);
  return idx !== -1 ? planText.slice(0, idx).trim() : planText.trim();
};

const splitPlanIntoDays = (planText: string): string[] => {
  const planOnly = getPlanWithoutClosingText(planText);
  const firstDayIdx = planOnly.search(/Day\s+\d+\s*:/i);
  const trimmed = firstDayIdx >= 0 ? planOnly.slice(firstDayIdx) : planOnly;
...
const getClosingText = (planText: string): string => {
  const idx = findClosingIndex(planText);
  return idx !== -1 ? planText.slice(idx).trim() : "";
};
    ...
    <View style={[styles.card, isOpen && styles.cardActive]}>
      <Pressable onPress={onToggle} style={styles.cardHeader}>
        <View style={[styles.dayBadge, isOpen && styles.dayBadgeActive]}>
          <Text style={[styles.dayBadgeNum, isOpen && styles.dayBadgeNumActive]}>
            {day.dayNumber}
          </Text>
          <Text style={[styles.dayBadgeLabel, isOpen && styles.dayBadgeLabelActive]}>
            DAY
          </Text>
        </View>
              ...
              {session.items.map((item, ii) => (
                <View key={ii} style={styles.drillItem}>
                  ...
                  <View style={styles.drillText}>
                    ...
                    {item.detail ? (
                      <Text style={styles.drillDetail}>{item.detail}</Text>
                    ) : null}
                  </View>
                </View>
              ))}
            ...
            <View style={styles.tagRow}>
              {day.tags.map((tag, ti) => (
                <View key={ti} style={[styles.tag, ti === 0 && styles.tagGreen]}>
                  <Text style={[styles.tagText, ti === 0 && styles.tagTextGreen]}>
                    {tag}
                  </Text>
                </View>
              ))}
...
function EmptyState({ onGenerate }: { onGenerate: () => void }) {
  return (
    <View style={styles.emptyState}>
      ...
      <Pressable style={styles.regenBtn} onPress={onGenerate}>
    ...
    try {
      ...
      if (data) {
        ...
        setUserStats({
          ...
          days: data.days_per_week ? Number(data.days_per_week) : 7,
    ...
    >
      ...
      {days.length > 0 ? (
        <>
          ...
          <Pressable style={styles.regenBtn} onPress={loadPlan}>
            <Text style={styles.regenBtnText}>↺  Regenerate Plan</Text>
          </Pressable>
        </>
      ) : (
        <EmptyState onGenerate={loadPlan} />
      )}
    </ScrollView>
  );
}
